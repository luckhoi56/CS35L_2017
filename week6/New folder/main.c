...
void * processImage(void* a)
{
    ...
    const double subsample_dy
        ...
        : pixel_dy;
    /* for every pixel */
    for (int px = *(int*)a; px<width; px += nthreads)
    {
...
main(int argc, char **argv)
{
    ...
    int *section = (int*)malloc(sizeof(int)* nthreads);
    
    for (int k = 0; k < nthreads; k++)
    {
        section[k] = k;
        int ret = pthread_create(&threads[k], NULL, processImage, &section[k]);
        if (ret)
        {
            fprintf(stderr, "Error when  threads");
            exit(1);
        }
    }
    for (int t = 0; t < nthreads; t++)
    {
        ...
        if (ret)
        {
            fprintf(stderr, "Error when release thread");
            exit(1);
        }
    }
